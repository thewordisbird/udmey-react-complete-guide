...
export const checkAuthTimeout = (expirationTime) => {
  console.log('[actions/auth, checkAuthTimeout]', expirationTime)
  return dispatch => {
    setTimeout(() => {
      dispatch(logout())
    }, expirationTime * 1000)
  ...
  return dispatch => {
    ...
    if (!token) {
      ...
      if (expDate < new Date()){
        ...
        console.log('[actions/auth, authCheckState]', expDate.getTime(), new Date().getTime(), expDate.getTime() - new Date().getTime())
